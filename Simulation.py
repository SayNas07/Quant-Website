...
st.title("Calculating: The Efficient Frontier & Capital Asset Line")

st.write("### Input Data")
col1, col2 = st.columns(2)
risk_free_rate = col1.number_input("Risk Free Rate (%)", min_value=0, value=2) / 100
num_tickers = col2.number_input("No. of tickers in the portfolio", min_value=2, max_value=20, value=3)
...
selected = []
attempts = 0
while len(selected) < num_tickers and attempts < 100:
    candidate = random.choice(sp100_tickers)
    if candidate not in selected:
        try:
            # Test if ticker has data
            test_data = yf.download(candidate, period="3mo", progress=False)
    ...
    attempts += 1
for i in selected:
    st.write(f"Selected Ticker: {i}")

# -------------------------
# DOWNLOAD DATA
# -------------------------
with st.spinner("Downloading data..."):
    data = yf.download(selected, start="2018-01-01", progress=False)["Close"]
